...
TODO: Consider adding list of allowed 'audiences'
  ...
  <MainTemplate v-if="client" :title="`Edit ${client.client_name}`">
    <template v-slot:header>
      ...
      <div
        ...
        role="alert"
      >
        The management APIs depend on this client for functioning. Only edit the
        properties if you know what you are doing.
      </div>
    </template>
    ...
    <template v-slot:body>
      ...
      >
        <FormGroup label="Application Type" horizontal :label-cols="3">
          <FormRadioGroup
          ...
          >
            <FormRadioButton value="web" v-model="client.application_type">Web Client</FormRadioButton>
            <FormRadioButton value="native" v-model="client.application_type" 
              >Native Client (such as a mobile web application)</FormRadioButton
            >
          </FormRadioGroup>

          <small id="application_type_help" class="form-text text-muted">
            Web clients require
            <code class="highlighter-rouge">https</code> redirection urls.
            Native clients allow custom app schemas.
          </small>
        </FormGroup>

        <FormGroup label="Public" horizontal :label-cols="3">
          <FormRadioGroup
            ...
            name="radios3Sub"
          >
            <FormRadioButton value="public" v-model="client.public">Public</FormRadioButton>
            <FormRadioButton value="confidential" v-model="client.public">Confidential</FormRadioButton>
          </FormRadioGroup>

          <small id="client_public_help" class="form-text text-muted">
            Confidential clients require a
            <code class="highlighter-rouge">client_secret</code> for exchanging
            an authorization code. Public clients do not.
          </small>
        </FormGroup>
        ...
        <div class="form-group form-row">
          ...
          <div class="col">
            ...
            <small id="client_idHelp" class="form-text text-muted"
              >Unique Client Identifier</small
            >
          </div>
        </div>
        ...
        <div class="form-group form-row">
          ...
          <div class="col">
            ...
            />
            <small id="client_nameHelp" class="form-text text-muted"
              >Name of the Client to be presented to the End-User.</small
            >
            <template v-if="errors != null && errors.client_name">
              <div
                v-for="e in errors.client_name"
                :key="e"
            ...
            <div
              v-if="errors != null && !errors.client_name"
              class="invalid-feedback"
            >
        ...
        <FormGroup label="PKCE supported" horizontal :label-cols="3">
          <FormRadioGroup
            ...
            name="pkce"
          >
            <FormRadioButton :value="null"  v-model="client.code_challenge_methods_supported">No</FormRadioButton>
            <FormRadioButton :value="['S256', 'plain']"  v-model="client.code_challenge_methods_supported">Yes</FormRadioButton>
          </FormRadioGroup>

          <small id="pkce_help" class="form-text text-muted">
            For native clients - such as mobile applications - you do want to
            enable
            <code class="highlighter-rouge">PKCE</code>.
          </small>
        </FormGroup>
        ...
        <div class="form-group form-row">
          ...
          <div class="col">
            <multiselect
              class="form-control"
              :class="{
                'is-invalid': errors != null && errors['redirect_uris.1'],
              }"
              :customLabel="(e) => e"
              ...
              @tag="
                (searchQuery, id) => {
                  client.redirect_uris.push(searchQuery);
                  if (!show_post_logout_redirect_uris) {
                    client.post_logout_redirect_uris = client.redirect_uris;
                  }
                }
              "
            ...
            <small id="pkce_help" class="form-text text-muted"
              ...
              <template v-if="!show_post_logout_redirect_uris"
                >By default, this list also determined the allowed post logout
                redirect uris.
                <a
                  href="#"
                  @click.prevent="show_post_logout_redirect_uris = true"
                  >Configure different logout redirect uris.</a
                ></template
              ></small
            >
        ...
        <div
          class="form-group form-row"
          v-show="show_post_logout_redirect_uris"
        >
          <label class="col-3" for="post_logout_redirect_uris"
            >Logout redirect URIs</label
          >
          ...
          <div class="col">
            <multiselect
              ...
              @tag="
                (searchQuery, id) => {
                  client.post_logout_redirect_uris.push(searchQuery);
                }
              "
              :close-on-select="false"
              :options="[]"
            ...
            <small id="pkce_help" class="form-text text-muted"
              >List of allowed post logout redirection uris. You may choose to
              <a
                href="#"
                v-show="show_post_logout_redirect_uris"
                @click.prevent="
                  ...
                  client.post_logout_redirect_uris = client.redirect_uris;
                "
                >use the list of redirect uris.</a
              ></small
            >
        ...
        <FormGroup label="Grant Types" horizontal :label-cols="3">
          <FormCheckboxGroup
            id="checkboxes1"
            name="flavour1"
          ...
          <small id="pkce_help" class="form-text text-muted"
            >List of the OAuth 2.0 Grant Types that the Client is declaring that
            it will restrict itself to using.</small
          >
        </FormGroup>

        <FormGroup label="Response Types" horizontal :label-cols="3">
          <FormCheckboxGroup
            id="response_types"
            name="response_types"
          ...
          <small id="pkce_help" class="form-text text-muted"
            >List of the OAuth 2.0 response_type values that the Client is
            declaring that it will restrict itself to using itself to
            using.</small
          >
        </FormGroup>
        ...
        <div class="form-group form-row">
          ...
          <div class="col">
            ...
            <small id="pkce_help" class="form-text text-muted"
              >URL that references a logo for the Client application. Visible on
              the login screen.</small
            >
          </div>
        </div>

        <div class="form-group form-row">
          <label class="col-3" for="client_uri"
            >URL of the home page of the Client.</label
          >
        ...
        <div class="form-group form-row">
          <label for="default_prompt" class="col-3"
            >Default prompt settings.</label
          >
              ...
              <div class="input-group-append">
                <FormSelect
                  ...
                  :options="[
                    ...
                    { value: false, text: 'Do NOT Allow Override' },
                  ]"
                  class="custom-select"
                />
              </div>
        ...
        <div class="form-group form-row">
          <label for="default_acr_values" class="col-3"
            >Default authentication context required.</label
          >
            ...
            <div class="input-group">
              <div class="col p-0">
                <multiselect
                  ...
                  :options="options"
                  :customLabel="
                    (option, label) => (label ? option.level : option.id)
                  "
                  label="level"
                  track-by="id"
                  :searchable="false"
        ...
        <p>
          A client itself may get assigned roles. This allows a client to call
          an API on behalf of itself, after obtaining an access token via a
        ...
        <div class="form-row">
          ...
          <div class="col">
            ...
            <p v-else>
              ...
              <router-link tag="a" class to="/groups"
    ...
    <template v-slot:footer>
      <Danger
        ...
        v-if="client && client.client_id != currentClientId"
      >
        <button
          type="button"
          class="btn btn-danger"
          @click="deleteObject(client)"
        >
          Delete
        </button>
      </Danger>
...
<script setup>
import { getDecodedAccesstoken, maxios, laxios } from "@/admin/helpers.js";
...
const groups = ref([]);
const options = ref([]);
const options_user_interfaces = ref([]);
const passwordType = ref("password");
const client = ref(null);
const currentClientId = ref(null);
const show_post_logout_redirect_uris = ref(false);
...
onMounted(() => {
  ...
  laxios
    .get(
      "/oauth/connect/register/" +
        encodeURIComponent(route.params.client_id)
    )
    .then(
      (response) => {
        client.value = response.data;

        if (!client.value.redirect_uris) {
          client.value.redirect_uris = [];
        }

        if (!client.value.post_logout_redirect_uris) {
          client.value.post_logout_redirect_uris = [];
        }

        if (
          JSON.stringify(client.value.redirect_uris) !=
          JSON.stringify(client.value.post_logout_redirect_uris)
        ) {
          show_post_logout_redirect_uris.value = true;
        }
  ...
  maxios.get("api/uiServers").then((response) => {
    options_user_interfaces.value = response.data;
    options_user_interfaces.value.push({
      url: "Built in UI",
      id: null,
    });
  });

  maxios.get("api/scim/v2/Roles").then((response) => {
...
function deleteObject(o) {
  laxios
    .delete(
      "oauth/connect/register/" + encodeURIComponent(this.client.client_id)
    )
    .then(() => {
      notify({
...
function onSubmit(event) {
  if (event.target.checkValidity()) {
    laxios
      .put(
        "oauth/connect/register/" + encodeURIComponent(client.value.client_id),
        client.value
      )
      .then(
        (response) => {
