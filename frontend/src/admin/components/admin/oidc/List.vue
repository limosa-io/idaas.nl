...
<template>
  <div class="container-fluid">
    ...
    <b-dropdown text right class="float-right">
      ...
      <a class="dropdown-item" :href="$oidcUrl('.well-known/jwks.json')" target="_blank">JWK</a>
    </b-dropdown>
    ...
    <!--
    TODO: Remove this test functionality. Better invest in the test application
    <router-link
      tag="button"
      class="btn btn-md btn-secondary float-right mr-2"
      to="/applications/oidc/test"
    >Test</router-link>
    -->
    ...
    <h4 class="c-grey-900 mt-1 mb-3">Applications</h4>
  ...
  <div class="d-flex flex-row justify-content-between">
          <div>
Manage your OpenID Connect applications.
          </div>

          <form class="form-inline" v-on:submit.prevent="onSubmit">
            
            <label class="sr-only" for="search.email">E-mail</label>
            <input type="search" class="form-control form-control-sm mb-2 mr-sm-2" id="search" v-model="search"
              placeholder="">
          ...
          <table class="table table-hover">
            ...
            <tbody>
              <tr v-for="(client,index) in clients_filtered" @click="edit(client)" :key="index">
                ...
                <td class="d-none d-lg-block">
                  <ul v-for="(uri,index) in client.redirect_uris" class="list-group" :key="index">
...
export default {
  ...
  computed: {
    clients_filtered: function(){
      return this.clients ? this.clients.filter( value => {
        console.log(value.client_id);
        return this.search == null || value.client_id.includes(this.search) || value.client_name.toLowerCase().includes(this.search.toLowerCase());
      }) : []
    }
  },
