...
class AuthChainSubscriber
{
    ...
    public function onAuthenticated(Authenticated $event)
    {
        ...
        /**
         * Depending on the registration settings, make a user active when he has reached a certain authentication level.
         */
        ...
        if($event->state->getSubject() != null &&  ($user = $event->state->getSubject()->getUser()) != null ) {
            ...
            if(!$user->active) {
                ...
                $level = $setting != null ? AuthLevel::find($setting->value) : AuthLevel::where('level', 'activation')->first();
        ...
        }else if($event->state->getSubject() != null) {
